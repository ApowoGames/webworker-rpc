...
export class RPCPeer extends RPCEmitter {
    ...
    public attach(workerName: string, workerUrl?: string, onlyOneWorker?: boolean, workerType?: WorkerType): LinkListener {
        ...
        if (onlyOneWorker) {
            ...
            const newWorker = new Worker(workerUrl, { type: workerType });
            ...
            if (!this.channels.has(MANAGER_WORKER_NAME)) {
                ...
                const managerWorker = new Worker(managerWorkerURL, { type: workerType });
    ...
    private updateRegistry() {
        ...
        for (const context of ExportedClasses) {
            if (!ExportedContexts.has(context)) {
                console.warn("webworker-rpc: Export only decorate Emitter! check class: " + context);
                continue;
            }
        ...
        for (const oneKey of attributeKeys) {
            ...
            if (!ExportedContexts.has(contextStr)) {
                console.warn("webworker-rpc: Export only decorate Emitter! check attribute: " + oneKey);
                continue;
            }
    ...
    private onMessage_GotRegistry(packet: webworker_rpc.IGotRegistryPacket) {
        ...
        if (this.syncRegistryListeners.has(id)) {
            ...
            this.syncRegistryListeners.delete(id);
        }
    }
    ...
    private onMessage_ProxyCreateWorker(packet: webworker_rpc.IProxyCreateWorkerPacket, ports: MessagePort[]) {
        const { workerName, workerUrl, workerType, msg } = packet;
